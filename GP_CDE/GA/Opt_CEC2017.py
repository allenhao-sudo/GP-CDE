...
def run_parallel(n_process, func, func_set, D, maxeval):
    Processes = []
    for i in range(n_process):
        if i < 10:
            func_id = int(func_set[func, 0])
            run_index = i
        elif i < 20:
            func_id = int(func_set[func, 1])
            run_index = i - 10
        else:
            func_id = int(func_set[func, 2])
            run_index = i - 40
        p = multiprocessing.Process(target=Opt_CEC2017, args=(run_index, [25, 20], func_id, D, maxeval))
        Processes.append(p)
        p.start()
...
if __name__ == "__main__":
    ...
    Opt_CEC2017(run_index=1, para=[25, 20], func_id=func_set[0], D=10, maxeval=500)
